...
| 阅读 | 夜灯纸本 · 桌面**程目**侧栏 · 手机**底坞**（上程/程目/下程）· 本机进度 |
| 文案 | 地域/村镇两行联，不写工程设定 |

字体：默认瘦金系正文 + 书法标题；案前可换。配色默认 **雾金**。  
首屏仅加载核心 web 字体（`display=optional` + 系统宋/楷兜底，减少跳字）；GSAP 与卷宗索引空闲后再挂。
...
- 左侧暗底雾金**程目**；按主路 / 旁支 / 铺地分段；`is-current` / `is-read` / 高峰 ★
- **展开/收回**：顶栏「程目 / 掩目」· 侧栏「掩」· **T**；桌面收起留窄轨；窄屏抽屉 + 遮罩，点选后自动收
- 偏好：`shanhe.yiwen.reader.toc.desk` / `.mobile`
- 列表序 = 当前阅读列表（默认 `reading_path`/`main_path` + 同卷余章）
- **≤860px（v0.46）**
  - 底坞 `#readerDock`：上程 / 程目 / 下程（顶栏隐藏入世与双程键）
  - 审印：扁条横滑（小印 · 状态 · 摘要胶囊 · 详/批）；点「详」展开全文
  - **不渲染脚链**「另见/碎片」条（挡读）；桌面仍可显示关联芯片
  - `viewport-fit=cover` + safe-area；`body.is-reading` 锁背景滚动；换章回顶
...
审查标准见 `docs/原则/21_故事审查标准.md` **v0.4**。
...
| **世界** | 碎片仓：势力/地标/生灵/物产/风物/**杂记**/人物/未解/卷宗摘；点卡开本碎片 + 关联 |
| **卷宗** | `docs/` 设定全文 |
| **时序** | 年号轴、未解之谜 |
| **案前** | 明暗、雾金等配色、字号字体、环境 |
...
- `stories/世界/` — 碎片唯一源（势力/地标/生灵/物产/风物/**杂记**）；非正文、非人物志全文
...
- 网格点卡 → **先读本卡 path**（不再误开 see[0] 章）
- 阅读器：顶条「碎片」印 · 底栏章/联/宗/回指 · 文末关联面板
- 人物分册：已立志 N 优先；正文名录折叠
- 新卡：`python3 scripts/scaffold_entry.py --world --id WLD-XX --kind scrap --name 某杂记`
...
| `#/read/Z007` | 直达章节阅读（示例） |
| `#/shard/WLD-QYBJ` | 世界碎片阅读 + 关联 |
| `#/doc/docs/...` | 打开卷宗 md |
| `#/seal/pending` | 审印阶段 |
| `#/world/factions` · `#/world/scraps` | 世界分册 |
